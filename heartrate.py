...
def load_segments(filename, verbose=True):
    ...
    """
    w = load_workbook(filename, read_only=True)
    sheet = w["Sheet1"]
    ...
    for row in sheet.iter_rows(min_row=2):
        ...
        if type(row[0].value) == str:
            a, b = row[0].value.split(".")
        else:
            a, b = "{:.1f}".format(row[0].value).split(".")
        ...
        segments[key_subj][key_seg]['on'] = int(row[1].value)
        segments[key_subj][key_seg]['off'] = int(row[2].value)
        # ECG band-pass frequencies
        segments[key_subj][key_seg]['freq_lo_ecg'] = float(row[3].value)
        segments[key_subj][key_seg]['freq_hi_ecg'] = float(row[4].value)
        # RESP band-pass frequencies
        segments[key_subj][key_seg]['freq_lo_resp'] = float(row[5].value)
        segments[key_subj][key_seg]['freq_hi_resp'] = float(row[6].value)
        # ECG & RESP thresholds
        segments[key_subj][key_seg]['thr_ecg'] = float(row[7].value)
        segments[key_subj][key_seg]['thr_resp'] = float(row[8].value)
        # maximum peak-to-peak frequency for ECG & RESP
        segments[key_subj][key_seg]['f_max_ecg'] = float(row[9].value)
        segments[key_subj][key_seg]['f_max_resp'] = float(row[10].value)
        if verbose:
...
def prepare_segments(verbose=True):
    """
    Load filter and threshold parameters from .xlsx file
    dictionary of subjects and segments, processing parameters
    """
    ...
    for i in range(1, 11):
        ...
        subj_set.add(i_subj)
        # segment start (on) and stop (off) indices
        segments[key_subj][key_seg]['on'] = 0
        segments[key_subj][key_seg]['off'] = 0
        # ECG band-pass frequencies
        segments[key_subj][key_seg]['freq_lo_ecg'] = 20
        segments[key_subj][key_seg]['freq_hi_ecg'] = 125
        # RESP band-pass frequencies
        segments[key_subj][key_seg]['freq_lo_resp'] = 0.5
        segments[key_subj][key_seg]['freq_hi_resp'] = 5
        # ECG & RESP thresholds
        segments[key_subj][key_seg]['thr_ecg'] = 0.2
        segments[key_subj][key_seg]['thr_resp'] = 0.3
        # maximum peak-to-peak frequency for ECG & RESP
        segments[key_subj][key_seg]['f_max_ecg'] = 10
        segments[key_subj][key_seg]['f_max_resp'] = 2.5
        if verbose:
            print(f"on: {segments[key_subj][key_seg]['on']:d}")
            print(f"off: {segments[key_subj][key_seg]['off']:d}")
            print(
